...
function formatTwelveDate(date: string) {
  // Input: 2025-01-01
...
async function fetchTwelveSeries(symbol: string, from: string, to: string) {
  ...
  url.searchParams.set("start_date", formatTwelveDate(from));
  url.searchParams.set("end_date", formatTwelveDate(to));
...
export async function GET(req: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          example:
            "/api/backfill-twelve?from=2025-01-01&to=2025-02-01&secret=test123",
        },
        { status: 400 },
      );
    }
    ...
    for (const pair of PAIRS) {
      series[pair] = await fetchTwelveSeries(pair, from, to);
    }
